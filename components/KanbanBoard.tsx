...
const KanbanBoard: React.FC<KanbanBoardProps> = ({ demands, citizens, onViewDemand, onUpdateStatus }) => {
  ...
  return (
    <div className="flex flex-col h-full animate-in fade-in zoom-in-95 duration-500 pb-24 md:pb-0">
        ...
        <div className="flex-1 overflow-x-hidden overflow-y-auto md:overflow-visible custom-scrollbar">
            {/* 
                Layout Logic:
            ...
            */}
            <div className="flex flex-col landscape:flex-row md:flex-row h-full landscape:gap-4 md:gap-6 pb-4">
                {columns.map(col => {
                    ...
                    return (
                        <div 
                            key={col.id} 
                            className={`${visibilityClass} flex-1 flex-col min-w-0 md:min-w-[320px] md:max-w-md h-full transition-all duration-300`}
                        ...
                        >
                            ...
                            <div className={`flex-1 overflow-y-auto p-2 landscape:p-2 md:p-3 space-y-3 border-x border-b border-slate-200 dark:border-white/10 rounded-b-3xl bg-white/30 dark:bg-white/5 scrollbar-thin scrollbar-thumb-slate-300 dark:scrollbar-thumb-white/10 transition-colors ${draggedDemandId ? 'bg-slate-50/50 dark:bg-white/10' : ''}`}>
                                {colDemands.length === 0 ? (
